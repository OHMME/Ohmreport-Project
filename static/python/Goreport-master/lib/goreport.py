...
Ensure the IDs are provided as comma-separated integers or interger ranges, e.g. 5,50-55,71.")
    ...
    def process_results(self, combine_reports):
        ...
        for target in self.campaign.results:
            ...
            #ohm
            if target.email in self.targets_sent:
                for event in self.timeline:
                    if event.message == "Email Sent" and event.email == target.email:
                        ...
                        temp_dict["sentdate"] = sentdate.strftime("%Y-%m-%d %H:%M:%S")
                        break
            ...
            if target.email in self.targets_opened:
                for event in self.timeline:
                    if event.message == "Email Opened" and event.email == target.email:
                        ...
                        opendate = dt.datetime(Open_Link_date[0], Open_Link_date[1], Open_Link_date[2], Open_Link_time[0], 
                                               Open_Link_time[1], Open_Link_time[2])+ timedelta(hours=7)
                        temp_dict["opendate"] = opendate.strftime("%Y-%m-%d %H:%M:%S")
                        break 
            else:
                temp_dict["opendate"] = "null"
            ...
            if target.email in self.targets_clicked:
                for event in self.timeline:
                    if event.message == "Clicked Link" and event.email == target.email:
                        ...
                        clickdate = dt.datetime(Click_Link_date[0], Click_Link_date[1], Click_Link_date[2], Click_Link_time[0], 
                                                Click_Link_time[1], Click_Link_time[2])+ timedelta(hours=7)
                        temp_dict["clickdate"] = clickdate.strftime("%Y-%m-%d %H:%M:%S")
                        break
            else:
                temp_dict["clickdate"] = "null"
            ...
            if target.email in self.targets_submitted:
                for event in self.timeline:
                    if event.message == "Submitted Data" and event.email == target.email:
                        ...
                        temp_dict["submitdate"] = submitdate.strftime("%Y-%m-%d %H:%M:%S")
                        break
            else:
                temp_dict["submitdate"] = "null"       
            # Append the temp dictionary to the event summary list
            self.campaign_results_summary.append(temp_dict)
    ...
    def process_timeline_events(self, combine_reports):
        ...
        for event in self.campaign.timeline:
            if event.message == "Email Sent":
                sent_counter += 1
                self.targets_sent.append(event.email)
    ...
    def write_word_report(self):
        ...
        # 2022-10-01T15:16:08.363441339Z
        # "2022-09-19T07:10:00Z"
        created_date = [ int(i) for i in self.created_date.split('T')[0].split('-')]
        created_time = [ int(i) for i in self.created_date.split('T')[1].split('.')[0].split(':')]
        self.created = dt.datetime(created_date[0], created_date[1], created_date[2], created_time[0], created_time[1], created_time[2])+ timedelta(hours=7)
        
        launch_date = [ int(i) for i in self.launch_date.split('T')[0].split('-')]
        launch_time = [ int(i) for i in self.launch_date.split('T')[1].split('.')[0].split('Z')[0].split(':')]
        self.launch = dt.datetime(launch_date[0], launch_date[1], launch_date[2], launch_time[0], launch_time[1], launch_time[2])+ timedelta(hours=7)
        p.add_run(f"""
Status: {self.cam_status}
Created: {self.created.strftime("%I:%M:%S on %Y-%m-%d %p")}
Started: {self.launch.strftime("%I:%M:%S on %Y-%m-%d %p")}
...
""")       
        ...
        for target in self.results:
            for event in self.timeline:                  
                if event.message == "Email Opened" and event.email == target.email:
                    ...
                    # 0:00:31
                    # 12 days, 7:53:14  
                    for _ in diffopen.split(","):
                        count +=1
                    if count == 1:
                        Count_minutes = int(diffopen.split(":")[1])
                        Count_hours = int(diffopen.split(":")[0])
                        if Count_minutes <=5:
                            self.email_open1.append(target.email)
                        ...
                        Count_minutes = int(diffopen.split(",")[1].split(":")[1])
                        if Count_minutes > 8 and Count_days <= 1:
        ...
        for target in self.results:
            for event in self.timeline:      
                if event.message == "Clicked Link" and event.email == target.email:
                    ...
                    # 12 days, 7:53:14                         
                    count = 0
                    for _ in diffclick.split(","):
                        count +=1
                    if count == 1:
                        Count_minutes = int(diffclick.split(":")[1])
                        Count_hours = int(diffclick.split(":")[0])
                        if Count_minutes <=5 and count:  
                            self.email_click1.append(target.email)
                        if Count_minutes > 5 and Count_minutes  <= 30:
                            self.email_click2.append(target.email)
                        if Count_minutes > 30 and Count_hours <= 8:
                            self.email_click3.append(target.email)                       
                    elif count == 2:
                        Count_days = int(diffclick.split(",")[0].split()[0])
                        Count_hours = int(diffclick.split(",")[1].split(":")[0])
                        Count_minutes = int(diffclick.split(",")[1].split(":")[1])
                        if Count_minutes > 8 and Count_days <= 1:
                            self.email_click4.append(target.email)
                        if Count_days > 1 and Count_days <= 7:
                            self.email_click5.append(target.email)
            ...
            for event in self.timeline:                 
                if event.message == "Submitted Data" and event.email == target.email:
                    ...
                    for _ in diffsubmit.split(","):
                        count +=1
                    if count == 1:
                        Count_minutes = int(diffsubmit.split(":")[1])
                        Count_hours = int(diffsubmit.split(":")[0])
                        if Count_minutes <=5 and count:
                            self.email_submit1.append(target.email)

                        if Count_minutes > 5 and Count_minutes  <= 30:
                            self.email_submit2.append(target.email)
                        if Count_minutes > 30 and Count_hours <= 8:
                            self.email_submit3.append(target.email)
       
                    elif count == 2:
                        Count_days = int(diffsubmit.split(",")[0].split()[0])
                        Count_hours = int(diffsubmit.split(",")[1].split(":")[0])
                        Count_minutes = int(diffsubmit.split(",")[1].split(":")[1])
                        if Count_minutes > 8 and Count_days <= 1:
                            self.email_submit4.append(target.email) 
                        if Count_days > 1 and Count_days <= 7:
                            self.email_submit5.append(target.email)
                    break
        ...
        run.bold = True
        count = 0         
        for target in self.email_open1:
            count += 1  
        run = p.add_run(f"""total {count} """)
        count = 0           
        for target in self.email_open1:
            count += 1
        ...
        d.add_page_break()
        d.add_heading("Awareness Training", 1)  
        ordered_results = self.campaign_results_summary
        #Opened: The user has opened the message but hasn't clicked on the phishing campaign link.
        p = d.add_heading(f"""Opened: The user has opened the message but hasn't clicked on the phishing campaign link""", 2)
        count = 0       
        for target in ordered_results:
            if target['opened'] == True and target['clicked'] == False:
                count += 1
                p = d.add_paragraph( f"{count}.{target['email']}")
        p = d.add_paragraph()
        # Clicked: The user has opened the message and clicked on the phishing campaign link.
        p = d.add_heading(f"""Clicked: The user has opened the message and clicked on the phishing campaign link""", 2)
        count = 0       
        for target in ordered_results:
            if target['opened'] == True and target['clicked'] == True:
                count += 1
                p = d.add_paragraph( f"{count}.{target['email']}")
        p = d.add_paragraph()
        # Submitted: The user opened the message, clicked on the phishing campaign link, and submitted their information. 
        p = d.add_heading(f"""Submitted: The user opened the message, clicked on the phishing campaign link, and submitted their information""", 2)
        count = 0
        for target in ordered_results:
            if target['opened'] == True and target['clicked'] == True and target['submitted'] == True:
                count += 1
                p = d.add_paragraph( f"{count}.{target['email']}")

        p = d.add_paragraph()
        #User Not Open
        p = d.add_heading(f"""User Not Open""", 2) 
        count = 0      
        for target in ordered_results:
            if target['opened'] == False:
                count += 1
                p = d.add_paragraph( f"{count}.{target['email']}")
        p = d.add_paragraph()
        ...
        p = d.add_paragraph()
        p = d.add_heading("",2)
        run = p.add_run('Top 10 User Risk Management')
        run.font.color.rgb = RGBColor.from_string('FF0000')
        count = 0
        for target in list(dict.fromkeys(self.risk)):
            count += 1
            p = d.add_paragraph(f"""{count}.{target}""")
            if count == 10:
                break
            ...
            if target.email in self.targets_clicked:
                ...
                for event in self.timeline:
                    if event.message == "Email Sent" and event.email == target.email:
                        ...
                        email_sent_run.text = f"""Email sent on {sentdate.strftime("%I:%M:%S at %Y-%m-%d %p")}"""
                    ...
                    if event.message == "Email Opened" and event.email == target.email:
                        ...
                        timestamp.text = opendate.strftime("%Y-%m-%d %I:%M:%S %z %p")
                        opened_counter += 1
                    ...
                    if event.message == "Clicked Link" and event.email == target.email:
                        ...
                        clickdate = dt.datetime(Click_Link_date[0], Click_Link_date[1], Click_Link_date[2], Click_Link_time[0], 
                                                Click_Link_time[1], Click_Link_time[2])+ timedelta(hours=7)
                        # timestamp.text = temp[0] + " " + temp[1].split('.')[0]
                        timestamp.text = clickdate.strftime("%Y-%m-%d %I:%M:%S %z %p")
                    ...
                    if event.message == "Submitted Data" and event.email == target.email:
                        ...
                        submitdate = dt.datetime(Submit_Link_date[0], Submit_Link_date[1], Submit_Link_date[2], 
                                                 Submit_Link_time[0], Submit_Link_time[1], Submit_Link_time[2])+ timedelta(hours=7)
                        # timestamp.text = temp[0] + " " + temp[1].split('.')[0]
                        timestamp.text = submitdate.strftime("%Y-%m-%d %I:%M:%S %z %p")
...
def parse_options(self, id, format, combine, complete, config, google, verbose):
    ...
    """
    # Print the Gophish bannere
    # Create a new Goreport object that will use the specified report format
    gophish = Goreport(format, config, google, verbose)
